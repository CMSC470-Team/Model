...
class SimpleLogreg(nn.Module):
    ...
    def forward(self, x):
        ...
        y_pred = torch.sigmoid(
            self.linear(x))  # sol Label has only two categories, so sigmoid and softmax should be essentially the same.
        return y_pred  # sol
        return 0.5
...
def step(epoch, ex, model, optimizer, criterion, inputs, labels):
    ...
    if (ex + 1) % 20 == 0:
        ...
        print(
            f'Epoch: {epoch + 1}/{num_epochs}, Example {ex}, loss = {loss.item():.4f}, train_acc = {acc_train.item():.4f} test_acc = {acc_test.item():.4f}')
...
if __name__ == "__main__":
    ...
    with open(args.vocab, 'r', encoding='utf8') as infile:
    ...
    criterion = None
    criterion = nn.BCELoss()  # sol
    optimizer = None
    optimizer = torch.optim.SGD(logreg.parameters(), lr=args.learnrate)  # sol
